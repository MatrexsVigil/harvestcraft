...
public class ContainerGrinder extends Container {
    ...
    public ContainerGrinder(InventoryPlayer inventory, TileEntityGrinder entityGrinder) {
        this.entityGrinder = entityGrinder;
        
        addSlotToContainer(new SlotPamGrinder(entityGrinder.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null), 0, 80, 23));
        addSlotToContainer(new SlotPamResult(entityGrinder.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null), 1, 62, 54));
        addSlotToContainer(new SlotPamResult(entityGrinder.getCapability(CapabilityItemHandler.ITEM_HANDLER_CAPABILITY, null), 2, 98, 54));
    ...
    public void detectAndSendChanges() {
        ...
        for (IContainerListener crafting : listeners) {
            if (lastCookTime == entityGrinder.cookTime) continue;
            crafting.sendWindowProperty(this, 0, entityGrinder.cookTime);
        }
        lastCookTime = entityGrinder.cookTime;
    }

    @Override
    public void updateProgressBar(int id, int data) {
        if (id == 0) {
        	entityGrinder.cookTime = (short) data;
        }
    }
